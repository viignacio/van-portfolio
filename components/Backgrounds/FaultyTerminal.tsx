...
export default function FaultyTerminal({
  ...
  const loadAnimationStartValue = useRef<number>(0);
  const frozenTimeValue = useRef<number>(0);
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      if (pageLoadAnimation && loadAnimationStartValue.current === 0) {
        loadAnimationStartValue.current = t;
      }

      if (!pause) {
        const elapsed = (t * 0.001 + timeOffsetRef.current) * timeScale;
        program.uniforms.iTime.value = elapsed;
        frozenTimeValue.current = elapsed;
      } else {
        program.uniforms.iTime.value = frozenTimeValue.current;
      }

      if (pageLoadAnimation && loadAnimationStartValue.current > 0) {
        const animationDuration = 2000;
        const animationElapsed = t - loadAnimationStartValue.current;
