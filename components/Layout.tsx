...
interface LayoutProps {
  ...
  footerData?: {
    title?: string;
    logo?: { asset?: { _ref?: string }; hotspot?: unknown; crop?: unknown };
    contentText?: string;
...
export default function Layout({ children, logoUrl, navbarData, footerData }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Footer */}
      {footerData && (
        <footer className="bg-base-800 text-white py-12 rounded-t-2xl">
          <div className="px-4 lg:w-4/5 lg:mx-auto">
            <div className="flex flex-col md:flex-row md:justify-between gap-12">

              {/* Brand */}
              <div className="flex flex-col gap-4 max-w-xs">
                {footerData.logo?.asset?._ref && (
                  <div className="relative h-12 w-12">
                    <Image
                      src={urlFor(footerData.logo).width(96).height(96).fit('fill').auto('format').url()}
                      alt="Logo"
                      fill
                ...
                {footerData.contentText && (
                  <p className="text-sm text-text-secondary leading-relaxed">{footerData.contentText}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-3 gap-16">
                ...
                {filteredNavLinks.length > 0 && (
                  <div>
                    <h3 className="mb-4 uppercase tracking-widest text-xs text-accent">Navigate</h3>
                    <ul className="space-y-3" role="list">
                      {filteredNavLinks.map((link, i) => (
                        <li key={i}>
                          {link.isExternal ? (
                            <a href={link.href} target="_blank" rel="noopener noreferrer" className="text-sm text-text-secondary lg:hover:text-accent transition-colors duration-200">
                              {link.label}
                            </a>
                          ) : link.href.includes('#') ? (
                            <a
                              href={link.href}
                              className="text-sm text-text-secondary lg:hover:text-accent transition-colors duration-200 cursor-pointer"
                              onClick={(e) => { e.preventDefault(); scrollToSection(link.href.replace(/.*#/, '')); }}
                            >
                              {link.label}
                            </a>
                          ) : (
                            <Link href={link.href} className="text-sm text-text-secondary lg:hover:text-accent transition-colors duration-200">
                              {link.label}
                            </Link>
                          )}
                        </li>
                ...
                {/* Social Media */}
                <div>
                  <h3 className="mb-4 uppercase tracking-widest text-xs text-accent">Follow Me</h3>
                  <ul className="space-y-3" role="list">
                    {footerData.socialMedia?.linkedin && (
                      <li>
                        <a href={footerData.socialMedia.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-text-secondary lg:hover:text-accent transition-colors duration-200 group">
                          <svg className="w-4 h-4 text-accent shrink-0 lg:group-hover:scale-110 transition-transform duration-200" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    {footerData.socialMedia?.github && (
                      <li>
                        <a href={footerData.socialMedia.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-text-secondary lg:hover:text-accent transition-colors duration-200 group">
                          <svg className="w-4 h-4 text-accent shrink-0 lg:group-hover:scale-110 transition-transform duration-200" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    {footerData.socialMedia?.facebook && (
                      <li>
                        <a href={footerData.socialMedia.facebook} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-text-secondary lg:hover:text-accent transition-colors duration-200 group">
                          <svg className="w-4 h-4 text-accent shrink-0 lg:group-hover:scale-110 transition-transform duration-200" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    {footerData.socialMedia?.twitter && (
                      <li>
                        <a href={footerData.socialMedia.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-text-secondary lg:hover:text-accent transition-colors duration-200 group">
                          <svg className="w-4 h-4 text-accent shrink-0 lg:group-hover:scale-110 transition-transform duration-200" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    {footerData.socialMedia?.instagram && (
                      <li>
                        <a href={footerData.socialMedia.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-text-secondary lg:hover:text-accent transition-colors duration-200 group">
                          <svg className="w-4 h-4 text-accent shrink-0 lg:group-hover:scale-110 transition-transform duration-200" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                ...
                {/* Contact Information */}
                <div>
                  <h3 className="mb-4 uppercase tracking-widest text-xs text-accent">Contact</h3>
                  <ul className="space-y-3" role="list">
                    {footerData.contactInfo?.email && (
                      <li className="flex items-center gap-3">
                        ...
                        <a href={`mailto:${footerData.contactInfo.email}`} className="text-sm text-text-secondary lg:hover:text-accent transition-colors duration-200">
                          {footerData.contactInfo.email}
                        </a>
                      </li>
                    )}
                    {footerData.contactInfo?.phone && (
                      <li className="flex items-center gap-3">
                        ...
                        <a href={`tel:${footerData.contactInfo.phone}`} className="text-sm text-text-secondary lg:hover:text-accent transition-colors duration-200">
                          {footerData.contactInfo.phone}
                        </a>
                      </li>
                    )}
                    {footerData.contactInfo?.location && (
                      <li className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-text-secondary">{footerData.contactInfo.location}</span>
                      </li>
                    )}
                  </ul>
                </div>
            ...
            <div className="border-t border-ui-border/30 mt-12 mb-8" />
